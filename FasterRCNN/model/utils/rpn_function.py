...
from model.utils import boxes as box_ops, det_utils
from torch.jit.annotations import List, Optional, Dict, Tuple
from torch import Tensor
from model.utils.image_list import ImageList
...
class AnchorsGenerator(nn.Module):
    ...
    image sizes.生成一组feature maps和图片大小的anchors
...
接下来，RPN会根据得分对anchor进行挑选，一般会选前景得分前topK的anchor
用delta对这些anchor进行偏移，然后再通过一步nms消除重叠面积较大的框，这些存活下来的优秀anchor会作为proposal递给rcnn head，再由head进行细分类和进一步矫正。
    Arguments:
